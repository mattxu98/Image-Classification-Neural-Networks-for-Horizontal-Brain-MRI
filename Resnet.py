...
def run_experiment(hyps, identifier):
    ...
    resnet18.classifier = new_classifier
    model = resnet18


    #... training code ...
    #Initialize lists to store metrics # CHANGE
    train_losses = []
    val_losses = []
    recalls = []
    f1s = []
    ...
    elif hyps['optimizer'] == 'rmsprop': #################################################
        optimizer = torch.optim.RMSprop(model.parameters(), lr=hyps['lr'])

    criterion = nn.CrossEntropyLoss() 
    num_epochs = 300 
    early_stopping = EarlyStopping(patience=100, min_delta=0.001) # Change patience and min_delta
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % validate_every == 0:
            ...
            model.eval()
            total_val_loss = 0.0
            all_labels = []  #Used to collect all real labels
            all_predictions = []  #Used to collect all predicted labels
            with torch.no_grad():
    ...
    for i, (title, y1, y2, ylabel) in enumerate(metrics, 1):
        ...
        plt.ylabel(ylabel)
    plt.tight_layout()
    plt.savefig(f'metrics_plots_{identifier}.png', dpi=300)  # Save the figure as a PNG file
    ...
    plt.savefig(f'confusion_matrix_{identifier}.png', dpi=300)  #Save the figure as a PNG file


#List of hyperparameter combinations
hyperparameter_combinations = [
    { 'lr': 0.001, 'dropout': 0.5, 'activation': 'relu', 'optimizer': 'adam' },
    { 'lr': 0.001, 'dropout': 0.3, 'activation': 'relu', 'optimizer': 'rmsprop' },
    { 'lr': 0.001, 'dropout': 0.1, 'activation': 'relu', 'optimizer': 'sgd' },
    { 'lr': 0.01, 'dropout': 0.3, 'activation': 'tanh', 'optimizer': 'adam' },
    { 'lr': 0.01, 'dropout': 0.1, 'activation': 'relu', 'optimizer': 'rmsprop' },
    { 'lr': 0.01, 'dropout': 0.5, 'activation': 'sigmoid', 'optimizer': 'sgd' },
    { 'lr': 0.1, 'dropout': 0.1, 'activation': 'relu', 'optimizer': 'adam' },
    { 'lr': 0.1, 'dropout': 0.5, 'activation': 'tanh', 'optimizer': 'rmsprop' },
    { 'lr': 0.1, 'dropout': 0.3, 'activation': 'sigmoid', 'optimizer': 'sgd' },
]
